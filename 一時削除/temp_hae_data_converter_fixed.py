"""
HAE Data Converter - HAEデータをCSV形式に変換（修正版）
運動データ累積処理問題修正
"""
...
class HAEDataConverter:
    ...
    CUMULATIVE_METRICS = {
        'step_count',           # 歩数：分単位→日単位累積
        'active_energy',        # 活動カロリー：分単位→日単位累積
        'basal_energy_burned',  # 基礎代謝：分単位→日単位累積
        'dietary_energy',       # 摂取カロリー：食事単位→日単位累積
        'protein',              # タンパク質：食事単位→日単位累積
        'carbohydrates',        # 糖質：食事単位→日単位累積
        'fiber',                # 食物繊維：食事単位→日単位累積
        'total_fat'             # 脂質：食事単位→日単位累積
    }
    ...
    def extract_metric_value(self, metric: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # 累積処理が必要なメトリクスの場合
        if name in self.CUMULATIVE_METRICS:
            total_value = 0
            sample_date = None
            ...
            print(f"[INFO] {name} 累積処理: {len(data_points)}個のデータポイント → 合計 {total_value}")
            ...
            # 最終値使用（体重、体脂肪率等）
            latest_point = data_points[-1]  # 最新のデータポイント
    ...
    def convert_hae_to_csv_row(self, hae_file: Path) -> Optional[Dict[str, Any]]:
        """HAE JSONファイルを1行のCSVデータに変換（累積処理対応版）"""
        try:
            ...
            print(f"[SUCCESS] HAEデータ変換完了（累積処理対応）: {date_value}")
            return csv_row
